...
type
  TfrmTrsReturSales = class(TDockForm)
    suiPanel1: TsuiPanel;
    lblTransaksi: TLabel;
...
procedure TfrmTrsReturSales.ArrangeColSize;
begin
...
//grid.ColWidths[colFakturId]:= 0;
  ...
  grid.ColWidths[colBarcode]:= 0;

end;

procedure TfrmTrsReturSales.Calculate;
var
  total, 
...
procedure TfrmTrsReturSales.Execute(vPurpose, ID: integer);
begin
  inherited;
...
procedure TfrmTrsReturSales.getItem(var AItemCode: string; AItemID: integer;
  AQty: double);
var
...
procedure TfrmTrsReturSales.getVendor(ACode: string; AID: integer);
...
procedure TfrmTrsReturSales.InitForm;
begin
  InitGrid;
...
procedure TfrmTrsReturSales.InitGrid;
begin
  ResetGrid(grid, 2, 19, 1,1, -1);
  ArrangeColSize;
end;

function TfrmTrsReturSales.isSaved: boolean;
var i: integer;
begin
  Trx.Clear;
  ...
  Trx.SalesMst.PasienID := txtSpec.Tag;
   Trx.SalesMst.TransType := TRANS_TYPE_SALES_RETUR;
//  Trx.SalesMst.Notes      := txtNotes.Text;
  Trx.SalesMst.Userid     := GlobalSystemUser.UserId;
  Trx.SalesMst.Subtotal   := StrFmtToFloatDef(txtSubtotal.Text,0);
//  Trx.SalesMst.Biaya      := StrFmtToFloatDef(txtBiaya.Text,0);
  Trx.SalesMst.Tax        := StrFmtToFloatDef(txtTax.Text,0);
  Trx.SalesMst.Total      := StrFmtToFloatDef(txtTotal.Text,0);
  ...
  else Result:= Trx.UpdateCashOnDB();

  if Result then LoadData(Trx.SalesMst.SalesID);
  tbtSave.Enabled:= not Result;   
end;

function TfrmTrsReturSales.isValid: boolean;
...
procedure TfrmTrsReturSales.RemoveRow;
begin
...
procedure TfrmTrsReturSales.SeekItem(ASearch: string);
...
procedure TfrmTrsReturSales.SeekVendor;
var id: integer;
begin
...
procedure TfrmTrsReturSales.SetDisableEnable;
begin
...
procedure TfrmTrsReturSales.gridCanEditCell(Sender: TObject;
...
procedure TfrmTrsReturSales.gridCellValidate(Sender: TObject;
...
procedure TfrmTrsReturSales.gridGetAlignment(Sender: TObject;
...
procedure TfrmTrsReturSales.gridGetEditorType(Sender: TObject;
...
procedure TfrmTrsReturSales.gridKeyDown(Sender: TObject;
...
procedure TfrmTrsReturSales.gridKeyPress(Sender: TObject;
  var Key: Char);
begin
  case grid.Col of
...
procedure TfrmTrsReturSales.getUnitList;
var
  i: integer;
...
procedure TfrmTrsReturSales.getUnit(Sender: TObject);
...
procedure TfrmTrsReturSales.txtSpecKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  case key of
    VK_F2 : SeekVendor;
...
procedure TfrmTrsReturSales.txtSpecKeyPress(Sender: TObject;
  var Key: Char);
begin
//  if key <> #13 then txtSpec.Tag:= 0;
end;

procedure TfrmTrsReturSales.btnSpecClick(Sender: TObject);
begin
  SeekVendor
end;

procedure TfrmTrsReturSales.dtpExprKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
...
procedure TfrmTrsReturSales.dtpDelyKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
...
procedure TfrmTrsReturSales.dtpTransactionKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
...
procedure TfrmTrsReturSales.rdbTaxClick(Sender: TObject);
begin
//  Calculate;
  SavePostState(True);
end;

procedure TfrmTrsReturSales.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case Key of
...
procedure TfrmTrsReturSales.FormClose(Sender: TObject;
...
procedure TfrmTrsReturSales.LoadData(ID: integer);
...
for i:= 0 to Trx.SalesDetail_Arr.Count-1 do begin
    ...
    grid.Ints[colID,      i+1]:= Trx.SalesDetail_Arr[i].SalesDetailID;
    grid.Ints[colItemId,  i+1]:= Trx.SalesDetail_Arr[i].MstItem.ItemID;
    //grid.Cells[colConvId,  i]:= detail.FieldValue(2);
    grid.Cells[colcode, i+1]:= Trx.SalesDetail_Arr[i].MstItem.ItemCode;
    ...
    grid.Cells[colUnit,    i+1]:= Trx.SalesDetail_Arr[i].MstItemConversion.MstMaster.MstName;
    grid.Cells[colQty,     i+1]:= FloatToStrFmt(Trx.SalesDetail_Arr[i].Quantity);
    grid.Cells[colHarga,   i+1]:= FloatToStrFmt(Trx.SalesDetail_Arr[i].Price);
...
procedure TfrmTrsReturSales.txtSubtotalValueValidate(Sender: TObject;
  value: String; var IsValid: Boolean);
begin
  AdvEdtFloatFmt(Sender, value);
end;

procedure TfrmTrsReturSales.tbtDeleteClick(Sender: TObject);
begin
  RemoveRow
end;

procedure TfrmTrsReturSales.tbtSaveClick(Sender: TObject);
begin
  if isValid then isSaved 
end;

procedure TfrmTrsReturSales.tbtNewClick(Sender: TObject);
var tag : integer;
begin
  tag := 302;
...
procedure TfrmTrsReturSales.txtDisc2KeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmTrsReturSales.txtDisc2Exit(Sender: TObject);
begin
  Calculate;
end;

procedure TfrmTrsReturSales.txtSpecEnter(Sender: TObject);
begin
  status_delete:= 1;
end;

procedure TfrmTrsReturSales.gridEnter(Sender: TObject);
begin
  status_delete:= 2;
end;

procedure TfrmTrsReturSales.tbtPostClick(Sender: TObject);
begin
  if TTrsSales.PostTrans(lblTransaksi.Tag,TRANS_TYPE_SALES_RETUR) then
    txtNotes.Tag:= 1;
  SetDisableEnable;
end;

procedure TfrmTrsReturSales.dtpTransactionChange(Sender: TObject);
begin
  SavePostState(True);
end;

function TfrmTrsReturSales.GetOrder(): boolean;
...
procedure TfrmTrsReturSales.SavePostState(state: boolean);
begin
  tbtSave.Enabled:= state;
  if state then tbtPost.Enabled:= False; //ngahaja
end;

procedure TfrmTrsReturSales.txtSpecChange(Sender: TObject);
...
procedure TfrmTrsReturSales.txtTaxKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 case Key of
    VK_F3: txtSpec.SetFocus;
...
procedure TfrmTrsReturSales.CalculateBuangPPN;
...
procedure TfrmTrsReturSales.CalculateManualPPN;
var total, disc2, addt: double;
begin
  total:= StrFmtToFloatDef(txtSubtotal.Text,0);
  total:= total - disc2;
...
procedure TfrmTrsReturSales.ToolButton1Click(Sender: TObject);
begin
...
procedure TfrmTrsReturSales.gridSelectCell(Sender: TObject; ACol, ARow: Integer;
  var CanSelect: Boolean);
begin
...
function TfrmTrsReturSales.printFooter(value: TStringList; idx,
...
function TfrmTrsReturSales.printHeader(value: TStringList;
...
procedure TfrmTrsReturSales.printOrder(ID: integer);
