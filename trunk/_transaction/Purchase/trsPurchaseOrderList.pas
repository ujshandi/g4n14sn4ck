...
uses Subroutines, MainMenu,
  ...
  LookupData, UConstTool, trsPurchaseOrderListPrint, trsSalesGania;
...
begin
   if purpose in [SALES_TYPE_POS] then
    grid.PopupMenu :=PopupMenu1
   else
     grid.PopupMenu := nil;
  InitFilter;
  Label3.Caption:= IfThen(purpose = TRANS_TYPE_ORDER, 'Tgl Kirim :', 'Tgl Expr :');
  if purpose in [SALES_TYPE_POS,SALES_TYPE_RETUR] then begin
      txtVendor.Text:= 'Semua Customer';
...
begin
  try
    ...
    else if purpose in [TRANS_TYPE_SALES,TRANS_TYPE_SALES_RETUR] then data:= TTrsSales.LoadRekap
    else Exit;
    ...
    else if purpose = TRANS_TYPE_SALES then col_idx:= 6//5
    else col_idx:= 7;
    ...
    if purpose = TRANS_TYPE_ORDER then col_date1:= 4
     ...
     else if purpose = TRANS_TYPE_SALES then col_date1:= 2
      else if purpose = TRANS_TYPE_SALES_RETUR then col_date1:= 2
    else col_date1:= 4;
    ...
    for i:= 1 to data.RecordCount do begin
      ...
      for j:= 0 to data.FieldsCount-1 do
        if (j >= (col_idx-1)) then
          ...
          if (purpose = TRANS_TYPE_RETUR) and (j=5) then
              grid.Cells[j+1,i]:= EkstrakString(data.FieldValue(j),';',1)
          else
    ...
    if purpose = TRANS_TYPE_RECEIPT then  begin
     ...
     else   if purpose = TRANS_TYPE_SALES then  begin
       grid.ColWidths[0]:= 18;
        grid.ColWidths[1]:= 0;
        grid.ColWidths[8]:= 0;
      {  grid.ColWidths[8]:= 0;
     ...
     else   if purpose = TRANS_TYPE_SALES_RETUR then  begin
       grid.ColWidths[0]:= 18;
        grid.ColWidths[1]:= 0;
        grid.ColWidths[6]:= 0;
        grid.ColWidths[7]:= 0;
...
//  GlobalPeriode.setPeriode2(cmbOpr.ItemIndex-1, cmbOpr2.ItemIndex-1, dtpAwal.Date, dtpAkhir.Date, dtpAwal2.Date, dtpAkhir2.Date);
  ...
  if purpose in [TRANS_TYPE_SALES,TRANS_TYPE_SALES_RETUR] then
...
begin
  case purpose of
    ...
    TRANS_TYPE_SALES  : if (ACol = 0) or (ACol > 4) then HAlign:= taRightJustify;
    TRANS_TYPE_SALES_RETUR  : if (ACol = 0) or (ACol > 4) then HAlign:= taRightJustify;
  end;
end;
...
var id: integer;
begin
  if purpose= TRANS_TYPE_SALES  then
...
procedure TfrmTrsPurchaseOrderList.returTrans;
var NewTransaction: TDockForm;
begin
 // if not TSystemAccess.isCan(CAN_EDIT,AktiveControl.Tag) then exit;
  if grid.Ints[colId, grid.Row] = 0 then exit;
  NewTransaction:= ShowForm(Purpose);
  NewTransaction.Execute(SALES_TYPE_RETUR, grid.Ints[colId, grid.Row]);
end;

procedure TfrmTrsPurchaseOrderList.Retur1Click(Sender: TObject);
begin
//  inherited;
returTrans;
