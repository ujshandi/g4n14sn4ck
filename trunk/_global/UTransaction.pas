...
uses
  ...
  UMaster, AdvGrid, uMysqlClient, UConst;
...
type
  ...
  public
    ...
    function UpdateCashOnDB(): boolean;
    ...
    class function PostTrans(ID,tipe: integer): boolean;
  ...
  public
    ...
    class function isExistNoFaktur(nofaktur:string):boolean;
    {class function GetID(ID: integer): string;}
  end;
  ...
  public
    ...
    function InsertOnDB(): boolean; reintroduce;
    function UpdateOnDB(): boolean; reintroduce;
    function InsertPengeluaranKas(): boolean; reintroduce;
    function UpdatePengeluaranKas(): boolean; reintroduce;
    function InsertTabungan(): boolean;
    function UpdateTabungan(): boolean;
    function SelectInDB: boolean; reintroduce;
    function GetNewNumber: string;
    function SelectDetail(ID: integer): TMysqlResult;
    class function DeleteTrans(ID: integer): boolean;
    class function LoadRekap: TMysqlResult;
    class function LoadRekapTabungan: TMysqlResult;
    class function LoadRekapKas: TMysqlResult;
    class function LoadRekapTabunganNasabah: TMysqlResult;
  ...
  public
    ...
    //kelola giro
    class function LoadTglBayarGiro(): TMysqlResult;
    class function LoadBayarGiro(): TMysqlResult;
    class function LoadBayarGiroHistory(): TMysqlResult;

    //pmeblian by date
    class function LoadTglBeli(): TMysqlResult;
    class function LoadTglBeliApotek(): TMysqlResult;
    class function LoadTglBeliHistori(): TMysqlResult;
//penjualan by date
...
//rugi/laba
    ...
    class function getPembelian(tipe:integer=TRANS_TYPE_RECEIPT): double;
    class function getPemakaian(): double;
    class function getPembelianDisc(tipe:integer=TRANS_TYPE_RECEIPT): double;
    class function getPembelianRetur(): double;
    class function getMutasi(): double;
    class function getPembayaran(tipe:integer=TRANS_TYPE_PEMBAYARAN): double;
...
uses UConstTool, UQueryPackage, EpsonUtils,
...
begin
  try
    BeginSQL;
    sale:= OpenSQL(
    'select t.trans_date, t.relation_id, t.gudang_id, t.amount, '+
...
begin
  try
    ...
    sale:= OpenSQL(
    'select t.trans_date, t.relation_id, t.gudang_id, t.total, '+
    ...
    ' union all '+
    'select t.trans_date, t.relation_id, d.gudang_id, t.total, '+
...
begin
  ...
  try
    ...
    'insert into trs_sales_mst ('+
      'trans_num, trans_type, trans_date, cashier_id, gudang_id, relation_id, '+
...
var sql:string;
begin
  sql := 'select count(*) from trs_holded_mst m inner join mst_relation_animal a on m.relation_id=a.relation_id and a.seq = m.animal_seq where m.relation_id = '+FormatSQLNumber(aRelationId);
  sql := 'select count(*) from mst_relation_animal a '+
    ' where seq not in (select animal_seq from trs_holded_mst m where a.relation_id=m.relation_id ) '+
...
//    ' and animal_seq not in (select seq from mst_relation_animal '+FormatSQLNumber(aHewanSeq);
  Result := getIntegerFromSQL(sql)=0;
  if Result then
    Result := getIntegerFromSQL('select count(*) from trs_holded_mst where relation_id = '+FormatSQLNumber(aRelationId))>0;
end;
...
class function TTrsSales.LoadRekap: TMysqlResult;
...
begin
  ...
  if GlobalFilter.TipeID <> '0' then
    filter:= filter + ' and t.trans_type = '+FormatSQLNumber(StrToInt(GlobalFilter.TipeID));
  ...
  sqL:=
    'select distinct '+
      't.sales_id, t.trans_num as ''No Trx'', t.trans_date as ''Tgl '+IfThen(GlobalFilter.TipeID=IntToStr(TRANS_TYPE_SALES_RETUR),'Retur', 'Jual')+' '', '+
      '(select relation_name from mst_relation r where r.relation_id= t.relation_id) as Customer, '+
      'case when status_id = 0 then ''Un Posted'' else ''Posted'' end as Status, '+//t.notes as ''Keterangan'','+
      't.Subtotal, t.Disc_mst, t.tax as Pajak, t.Total '+
    'from trs_sales_mst t '+

    IfThen(GlobalFilter.SpecId > 1,
    ', trs_sales_detail d, mst_item i, point_maping m where d.sales_id = t.sales_id '+
    ' and d.item_id = i.item_id and i.specification = m.code_id ')+filter;
  Result:= OpenSQL(sqL);

end;

class function TTrsSales.PostTrans(ID, tipe: integer): boolean;
var i: integer; detail: TMysqlResult; msg,sql: string;
begin
  try
    BeginSQL;
    ExecSQL(
    'update trs_sales_mst set '+
    'status_id = 1, sisa = total where sales_id = '+FormatSQLNumber(ID));
  ...
  //  '((d.price/d.conversion) + (d.tax/d.quantity)) as harga, '+
   ...
   save_balance(CENTRAL_BOARD, BufferToInteger(detail.FieldValue(0)), BufferToDateTime(detail.FieldValue(2)),
       BufferToFloat(detail.FieldValue(5)), 0, IfThen(BufferToInteger(detail.FieldValue(6))=SALES_TYPE_POS,DB_CR_TYPE_CREDIT,DB_CR_TYPE_DEBIT)) ;
   {      save_balance(
   ...
   { if (detail.RecordCount > 0) and (tipe=TRANS_TYPE_RECEIPT) then
      save_to_relation_balance(
...
begin
 sql := 'select (d.quantity) as qty, d.price, d.amount, i.item_name, '+
  ...
  't.relation_id, t.dokter_id,0 as tipe_racikan,  0 as id_paket,  now() '+
  ...
  't.relation_id, t.dokter_id, 1 as tipe_racikan, d.paket_id as id_paket, now() '+
  ...
  for i:= 0 to buffer.RecordCount -1 do begin
    ...
    SalesDetail_Arr.Add(
      ...
      SalesDetail_Arr[i].MstItem.Barcode:= BufferToString(buffer.FieldValue(13));
       SalesDetail_Arr[i].MstItem.ItemCode:= BufferToString(buffer.FieldValue(13));
    buffer.MoveNext;
  end;
  buffer.Destroy;
end;
...
begin
  ...
  'subtotal, disc_mst, disc_val_mst, embalase, total, relation_id, gudang_id, points, dokter_id,status_id '+
  'from trs_sales_mst '+
  ...
  if Result then begin
    ...
    SalesMst.StatusId  := BufferToInteger(buffer.FieldValue(12));
  end;
  buffer.Destroy;
  ...
  //13qty_bns, 14conv_bonus, 15mst_code, 16nama_struk, 17barcode, 18points ,19hrg_beli,20service_type,21is_free,22animal_id
    ...
    SalesDetail_Arr.Add(
      ...
      SalesDetail_Arr[i].MstItem.ItemCode:= BufferToString(buffer.FieldValue(17));
      buffer.MoveNext;
  end;
  buffer.Destroy;
end;
...
function TTrsSales.UpdateCashOnDB: boolean;
var i,service_type: integer; sql: string;
  sales_value, dis_value, retur_value, jml_tagihan: double;
begin
  if DEMO_VERSION then begin
  ...
  try
    ...
    ' update trs_sales_mst set '+
      'subtotal = '+FormatSQLNumber(SalesMst.Subtotal)+','+
      ' profit = '+FormatSQLNumber(SalesMst.Profit)+','+
      ' disc_mst= '+FormatSQLNumber(SalesMst.DiscMst)+','+
      ' disc_val_mst= '+FormatSQLNumber(SalesMst.DiscValMst)+','+
      ' disc_detail= '+FormatSQLNumber(SalesMst.DiscDetail)+','+
      ' tax= '+ FormatSQLNumber(SalesMst.Tax)+','+
      ' total= '+ FormatSQLNumber(SalesMst.Total)+','+
      ' embalase= '+FormatSQLNumber(SalesMst.Embalase)+','+
      ' real_amount = '+ FormatSQLNumber(SalesMst.RealAmount)+','+
      ' sisa = '+ FormatSQLNumber(jml_tagihan)+','+
      ' cash = '+ FormatSQLNumber(SalesMst.Cash)+
      ' where sales_id = '+FormatSQLNumber(SalesMst.SalesID);
    ...
    ExecSQL(sql);
        ...
        sql :=  'delete from trs_sales_detail where sales_id = '+FormatSQLNumber(SalesMst.SalesID);
       ExecSQL(sql);
  //insert into detail
   ...
   //  if SalesDetail_Arr[i].PaketID<>0 then
     //   service_type := TMstService.getServiceType(SalesDetail_Arr[i].PaketID);
   //  if (SalesDetail_Arr[i].SalesDetailID = 0) then begin
      sql:=
        'insert into trs_sales_detail ('+
          'sales_id, item_id, quantity, price, disc, disc_val, amount, '+
          'amount_profit, price_purchase, paket_id, points, hrg_beli, conversion,service_type,is_free,animal_id)'+
...
class function TTrsReceipt.isExistNoFaktur(nofaktur: string): boolean;
var sql:string;
begin
  sql := 'select count(*) from trs_receipt_mst where trim(notes) = trim('+FormatSQLString(nofaktur)+')';
...
begin
  ...
  sqL:=
    'select distinct '+
      ...
      'case when tipe=0 then ''Terima PO'' when tipe=4 then ''Pembelian Langsung'' else ''Pembelian'' end as ''Tipe'', '+
...
class function TTrsReceipt.PostTrans(ID,tipe: integer): boolean;
...
begin
  try
    ...
    't.gudang_id, t.trans_date, t.total, t.relation_id, d.faktur_id,t.notes '+
    'from trs_retur_mst t, trs_retur_detail d '+
    'where d.retur_id = t.retur_id and t.retur_id = '+FormatSQLNumber(ID));

    detail.First;
    for i:= 0 to detail.RecordCount-1 do begin
    nofaktur := EkstrakString(detail.FieldValue(7),';',2);
    receiptid := getIntegerFromSQL('select receipt_id from trs_receipt_mst where trim(notes) = '+FormatSQLString(nofaktur));
      save_balance(
        CENTRAL_BOARD,//BufferToInteger(detail.FieldValue(2)),
        BufferToInteger(detail.FieldValue(0)),
        BufferToDateTime(detail.FieldValue(3)),
...
begin
  ...
  try
    ...
    'subtotal, discount, biaya, total, notes,status_id, user_id) '+
    'values ('+
      FormatSQLString(TransNum)+','+
      FormatSQLDate(Transdate)+','+
      FormatSQLNumber(Tipe)+','+
...
begin
  try
    ...
    'status_id = 1 where payment_id = '+FormatSQLNumber(ID));
...
begin
  Result:= OpenSQL(
  'select trans_num, relation_id, sisa from trs_sales_mst '+
...
begin
  ...
  'where t.trans_Type = '+FormatSQLNumber(TRANS_TYPE_PENGELUARAN_KAS)+filter;
...
class function TReport.getPembelian(tipe:integer=TRANS_TYPE_RECEIPT): double;
var sqL, filter: string;
begin
  filter := ' and t.tipe ='+FormatSQLNumber(tipe);
  if GlobalPeriode.PeriodeAwal1 <> 0 then
    filter:= filter + ' and t.trans_date '+FormatSQLOperator(GlobalPeriode.OpPeriodeAwal1)+FormatSQLDate(GlobalPeriode.PeriodeAwal1);
  if GlobalPeriode.PeriodeAkhir1 <> 0 then
    filter:= filter + ' and t.trans_date '+FormatSQLOperator(GlobalPeriode.OpPeriodeAkhir1)+FormatSQLDate(GlobalPeriode.PeriodeAkhir1);
...
class function TReport.getPembelianDisc(tipe:integer=TRANS_TYPE_RECEIPT): double;
var sqL, filter: string;
begin
  filter := ' and t.tipe ='+FormatSQLNumber(tipe);
  if GlobalPeriode.PeriodeAwal1 <> 0 then
...
begin
  ...
  '(select relation_name from mst_relation where relation_id = i.vendor_id) as spl, '+
  '(select mst_name from mst_master where mst_id = i.produsen_id) as produsen, '+
  'ifnull(sum(t.quantity),0) as sum_qty '+

  'from point_maping p, mst_item i, vie_report_sales t '+
  'where i.item_id = t.item_id and i.specification = p.code_id '+
...
begin
  ...
  sqL:=
  'select r.relation_id, r.relation_name as ''Nama Customer'', 0 as ''Saldo Awal'', '+
  ...
  '0 as ''Saldo Akhir'' '+
  'from mst_relation r inner join trs_sales_mst t on t.relation_id = r.relation_id and t.sisa > 0 where r.relation_type = '+FormatSQLNumber(RELASI_TYPE_CUSTOMER)+
...
begin
  ...
  '(select ifnull(sum(t.total),0) from trs_receipt_mst t '+filter_trans+'  and t.status_id = 1 and t.tipe = '+FormatSQLNumber(TRANS_TYPE_RECEIPT)+') as Pembelian, '+
  '(select ifnull(sum(t.total),0) from trs_retur_mst t '+filter_trans+'  and t.status_id = 1 ) as Retur, '+
  '(select ifnull(sum(t.subtotal + t.discount),0) from trs_payment_mst t '+filter_trans+filter_pay+'  and t.status_id = 1 ) as Pembayaran, '+
  '(select ifnull(sum(t.amount),0) from trs_debt_adjustment t '+filter_trans+') as Penyesuaian, '+
  '0 as ''Saldo Akhir'' '+
...
begin
 ...
 if GlobalFilter.FString1 <> '' then
  ...
  ',i.buying_price as ''H.Beli Terakhir'' '+
  'from mst_item i, point_maping p '+
  'where i.disabled_date is null and i.specification = p.code_id '+ fil+
  ' order by item_name ';
  Result:= OpenSQL(sqL);
...
begin
  ...
  '(select nama from mst_pasien r where r.relation_id = t.relation_id) as Pasien, '+
...
'    select -1* sum(t.total - t.embalase+ ifnull(t.dp,0)) as net, -1* sum(t.disc_val_mst) as embalase, -1* sum(t.profit) as profit, count(*)  as cust '+
  ...
  '  ( select sum(d.amount) from trs_sales_mst t  inner join trs_sales_detail d on d.sales_id = t.sales_id left join mst_item i on i.item_id = d.item_id where d.paket_id is not null and t.trans_type ='+FormatSQLNumber(SALES_TYPE_POS)+fil+')  as jasa '+
    ...
    ' union all '+
'    select -1* sum(t.total - t.embalase+ ifnull(t.dp,0)) as net, -1* sum(t.disc_val_mst) as embalase, -1* sum(t.profit) as profit, count(*)  as cust '+
    ',case when d.paket_id is null and ifnull(i.kategori,0)=0 then -1*sum(d.amount) else 0 end as barang, '+
    'case when d.paket_id is null and ifnull(i.kategori,0)=1 then -1*sum(d.amount) else 0 end as hewan, '+
  ' case when d.item_id is null and paket_id is not null then -1*sum(d.amount) else 0 end as jasa,'+
  ' (select -1*sum(d.hrg_beli * d.quantity) from trs_sales_mst t  inner join trs_sales_detail d on d.sales_id = t.sales_id  inner join mst_item i on i.item_id = d.item_id '+
    ' where t.trans_type ='+FormatSQLNumber(SALES_TYPE_RETUR)+fil+' and (i.kategori is null or i.kategori =0)) as barang_pokok, '+
  ' 0 as hewan_pokok '+
      'from trs_sales_mst t inner join trs_sales_detail d on d.sales_id = t.sales_id left join mst_item i on i.item_id = d.item_id where t.trans_type ='+FormatSQLNumber(SALES_TYPE_RETUR)+fil+
      ' ) as t1';
  Result:= OpenSQL(sql);
...
class function TReport.LoadTglBeli: TMysqlResult;
var sql:string;
begin
   sql :=
  'select distinct trans_date from trs_receipt_mst '+
  'where status_id = 1 and trans_date '+FormatSQLOperator(GlobalPeriode.OpPeriodeAwal1)+FormatSQLDate(GlobalPeriode.PeriodeAwal1)+
  IfThen(GlobalPeriode.PeriodeAkhir1 <> 0,
  ' and trans_date '+FormatSQLOperator(GlobalPeriode.OpPeriodeAkhir1)+FormatSQLDate(GlobalPeriode.PeriodeAkhir1))+
   IfThen(GlobalFilter.StatusID <> 0,
  ' and relation_id ='+FormatSQLNumber(GlobalFilter.StatusID))+
  ' union  '+
  'select distinct trans_date from trs_retur_mst '+
  'where status_id = 1 and  trans_date '+FormatSQLOperator(GlobalPeriode.OpPeriodeAwal1)+FormatSQLDate(GlobalPeriode.PeriodeAwal1)+
  IfThen(GlobalPeriode.PeriodeAkhir1 <> 0,
  ' and trans_date '+FormatSQLOperator(GlobalPeriode.OpPeriodeAkhir1)+FormatSQLDate(GlobalPeriode.PeriodeAkhir1))+
   IfThen(GlobalFilter.StatusID <> 0,
...
class function TReport.LoadTglBeliApotek: TMysqlResult;
var fil,sql: string;
begin
  fil:= '  t.trans_date = '+FormatSQLDate(GlobalPeriode.PeriodeAwal1);
  if GlobalFilter.Numeric2 <> 0 then
    fil:= fil + ' and t.cashier_id ='+FormatSQLNumber(GlobalFilter.Numeric2);
  if GlobalFilter.SeriesID <> 0 then
    fil:= fil + ' and t.shift_id ='+FormatSQLNumber(GlobalFilter.SeriesID);
...
//  if GlobalFilter.Numeric1 <> -1 then
  //  fil:= fil + ' and t.cara_bayar ='+FormatSQLNumber(GlobalFilter.Numeric1);
    ...
    sql := ' select sum(net),sum(spl),sum(barang),sum(retur) from ( '+
    ' select sum(t.total) as net,  count(*) as spl '+
    ', (select sum(d.amount) from trs_receipt_mst t  inner join trs_receipt_detail d on d.receipt_id = t.receipt_id  inner join mst_item i on i.item_id = d.item_id '+
    ' where '+fil+' and (i.kategori is null or i.kategori =0)) as barang, 0 as retur '+
     ' from trs_receipt_mst t where '+fil+
    ' union all '+
'    select -1* sum(t.total ) as net, count(*)  as spl '+
    ',0 as barang ,(select sum(d.amount) barang '+
      'from trs_retur_mst t inner join trs_retur_detail d on d.retur_id = t.retur_id left join mst_item i on i.item_id = d.item_id where '+fil+ //t.trans_type ='+FormatSQLNumber(TRANS_TYPE_RETUR)+fil+
      ' ) as retur '+
      ' from trs_retur_mst t where '+fil + ')'+
     '  as t1';
  Result:= OpenSQL(sql);
...
class function TReport.LoadTglBeliHistori: TMysqlResult;
var fil,sql: string;
begin
  if GlobalPeriode.PeriodeAwal1 <> 0 then
    fil:= fil + ' and t.trans_date '+FormatSQLOperator(GlobalPeriode.OpPeriodeAwal1)+FormatSQLDate(GlobalPeriode.PeriodeAwal1);
  ...
  sql :=
  'select concat(t.trans_num) as trans_num, i.barcode, i.item_name, '+
  //'round(d.price / d.conversion) as price, (d.quantity * d.conversion) as quantity, '+
  'd.price, (d.quantity / d.conversion) as quantity, '+
  'u.mst_code, d.disc1 as disc, d.amount, '+
  't.trans_date, t.subtotal, t.disc_mst, (t.total), d.receipt_detail_id as seq,  t.user_id,t.tipe '+

  'from trs_receipt_detail d, trs_receipt_mst t, mst_master u, mst_item i '+
  'where d.receipt_id = t.receipt_id and d.item_id is not null '+
  'and i.kemasan_id = u.mst_id and d.item_id = i.item_id '+fil+
  ' UNION all '+
  'select concat(t.trans_num,case when t.tipe='+FormatSQLNumber(TRANS_TYPE_RETUR)+' then ''(RETUR)'' else '''' end ) as trans_num, i.barcode, i.item_name, '+
  //'round(d.price / d.conversion) as price, (d.quantity * d.conversion) as quantity, '+
  'd.price, (d.quantity / d.conversion) as quantity, '+
  'u.mst_code, d.disc, d.amount, '+
  't.trans_date, t.subtotal, 0 as disc_mst,  (t.total), d.retur_detail_id as seq,  t.user_id,t.tipe '+

  'from trs_retur_detail d, trs_retur_mst t, mst_master u, mst_item i '+
  'where d.retur_id = t.retur_id and d.item_id is not null '+
  'and i.kemasan_id = u.mst_id and d.item_id = i.item_id '+fil+
  ' order by trans_date, trans_num, seq';
  Result:= OpenSQL(sql);


end;

class function TReport.LoadTglBayarGiro: TMysqlResult;
var sql:string;
begin
   sql :=
  'select distinct trans_date from trs_payment_mst t, trs_payment_detail d '+
  'where t.payment_id = d.payment_id and d.jenis_bayar = 2 and trans_date '+FormatSQLOperator(GlobalPeriode.OpPeriodeAwal1)+FormatSQLDate(GlobalPeriode.PeriodeAwal1)+
  IfThen(GlobalPeriode.PeriodeAkhir1 <> 0,
  ' and trans_date '+FormatSQLOperator(GlobalPeriode.OpPeriodeAkhir1)+FormatSQLDate(GlobalPeriode.PeriodeAkhir1))+
   IfThen(GlobalFilter.StatusID <> 0,
  ' and t.relation_id ='+FormatSQLNumber(GlobalFilter.StatusID))+
{  ' union  '+
  'select distinct trans_date from trs_retur_mst '+
  'where trans_date '+FormatSQLOperator(GlobalPeriode.OpPeriodeAwal1)+FormatSQLDate(GlobalPeriode.PeriodeAwal1)+
  IfThen(GlobalPeriode.PeriodeAkhir1 <> 0,
...
class function TReport.LoadBayarGiro: TMysqlResult;
var fil,sql: string;
begin
  fil:= '  t.trans_date = '+FormatSQLDate(GlobalPeriode.PeriodeAwal1);
  if GlobalFilter.Numeric2 <> 0 then
...
//  if GlobalFilter.Numeric1 <> -1 then
  //  fil:= fil + ' and t.cara_bayar ='+FormatSQLNumber(GlobalFilter.Numeric1);
    ...
    sql := ' select sum(t.total) as net,  count(*) as spl '+
    ' from trs_payment_mst t  inner join trs_payment_detail d on d.payment_id = t.payment_id '+
    ' where '+fil;
  Result:= OpenSQL(sql);
end;

class function TReport.LoadBayarGiroHistory: TMysqlResult;
var fil,sql: string;
begin
  if GlobalPeriode.PeriodeAwal1 <> 0 then
    fil:= fil + ' and t.trans_date '+FormatSQLOperator(GlobalPeriode.OpPeriodeAwal1)+FormatSQLDate(GlobalPeriode.PeriodeAwal1);
  ...
  sql := 'select trans_date,dok_num,bank,amount,nofaktur,jenistrx,tipe,tgl_cair from ('+
	' select t.trans_date,d.dok_num,d.bank,d.amount,r.trans_num as nofaktur, ''Bayar Utang'' as jenistrx,t.tipe,d.tgl_cair '+
	' from trs_payment_mst t inner join trs_payment_detail d on t.payment_id=d.payment_id inner join trs_receipt_mst r on t.inv_id = r.receipt_id'+
	' where t.status_id = 1 and t.tipe = '+FormatSQLNumber(TRANS_TYPE_PEMBAYARAN)+' and d.jenis_bayar = 2 '+fil+
' union                                                         '+
	' select t.trans_date,d.dok_num,d.bank,d.amount,r.trans_num as nofaktur, ''Terima Piutang'' as jenistrx,t.tipe,d.tgl_cair '+
	' from trs_payment_mst t inner join trs_payment_detail d on t.payment_id=d.payment_id inner join trs_sales_mst r on t.inv_id = r.sales_id'+
	' where t.status_id = 1 and t.tipe = '+FormatSQLNumber(TRANS_TYPE_PIUTANG)+' and d.jenis_bayar = 2 '+fil+
' ) as t1 '+
 ' order by trans_date, dok_num ';
  Result:= OpenSQL(sql);

end;

class function TReport.getPembayaran(tipe: integer): double;
var sqL, filter: string;
begin
  filter := ' and t.tipe ='+FormatSQLNumber(tipe);
  if GlobalPeriode.PeriodeAwal1 <> 0 then
  ...
  if GlobalPeriode.PeriodeAkhir1 <> 0 then
    filter:= filter + ' and t.trans_date '+FormatSQLOperator(GlobalPeriode.OpPeriodeAkhir1)+FormatSQLDate(GlobalPeriode.PeriodeAkhir1);
  if (tipe=TRANS_TYPE_PEMBAYARAN) then //tunai saja tanpa giro
      sqL:= 'select ifnull(sum(total),0) from trs_payment_mst t inner join trs_payment_detail d on d.payment_id = t.payment_id and d.jenis_bayar=1 '+
      IfThen(filter <> '', ' where ' + copy(filter, 5, length(filter)))
  else
    sqL:= 'select ifnull(sum(total),0) from trs_payment_mst t '+
  IfThen(filter <> '', ' where ' + copy(filter, 5, length(filter)));
  Result:= getFloatFromSQL(sqL);
...
begin
  ...
  try
    ...
    ' total, notes, person_id,insert_log) '+
    'values ('+
      FormatSQLString(TransNum)+','+
      FormatSQLDate(Transdate)+','+
...
function TTrsMoneyTransaction.InsertPengeluaranKas: boolean;
var i: integer; sqL: string;
begin
  if DEMO_VERSION then begin
    if getIntegerFromSQL('select count(*) from trs_money_trans_mst') > 100 then begin
...
function TTrsMoneyTransaction.InsertTabungan: boolean;
var i: integer; sqL: string;
begin
  if DEMO_VERSION then begin
    if getIntegerFromSQL('select count(*) from trs_money_trans_mst') > 100 then begin
  ...
  try
    ...
    'insert into trs_money_trans_mst ('+
    'trans_num, trans_date, trans_type, '+
    ' total, notes, relation_id,insert_log) '+
    'values ('+
      FormatSQLString(TransNum)+','+
      FormatSQLDate(Transdate)+','+
      FormatSQLNumber(TransType)+','+
    ...
    for i:= 0 to trsDetail.Count-1 do  begin
        ...
        'values ('+
          ...
          FormatSQLString(trsDetail[i].Description)+','+
          FormatSQLNumber(trsDetail[i].Amount)+','+
          IfThen(trsDetail[i].AkunID <> 0, FormatSQLNumber(trsDetail[i].AkunID), FormatSQLNULL)+','+
          //FormatSQLNumber(trsDetail[i].AkunID)+','+
          FormatSQLNumber(trsDetail[i].MoneyID)+')';
        ExecSQL(sql, False,sqL);
        save_to_relation_balance(PersonId,TransDate,trsDetail[i].Amount,IfThen(TransType=1,DB_CR_TYPE_DEBIT,DB_CR_TYPE_CREDIT));
//        save_money_balance(trsDetail[i].MoneyID,TransDate,trsDetail[i].Amount,IfThen(TransType=1,DB_CR_TYPE_DEBIT,DB_CR_TYPE_CREDIT));
    end;
    EndSQL;
    Result:= True;
...
class function TTrsMoneyTransaction.LoadRekapKas: TMysqlResult;
var sqL,filter: string;
begin
  filter := ' and t.trans_type in ('+IntToStr(TRANS_TYPE_PENGELUARAN_KAS)+') ';
  if GlobalPeriode.PeriodeAwal1 <> 0 then
    filter:= filter + ' and t.trans_date '+FormatSQLOperator(GlobalPeriode.OpPeriodeAwal1)+FormatSQLDate(GlobalPeriode.PeriodeAwal1);
  if GlobalPeriode.PeriodeAkhir1 <> 0 then
    filter:= filter + ' and t.trans_date '+FormatSQLOperator(GlobalPeriode.OpPeriodeAkhir1)+FormatSQLDate(GlobalPeriode.PeriodeAkhir1);
  ...
  sqL:=
    'select distinct '+
      't.money_trans_id, t.trans_num as ''No Trx'', t.trans_date as ''Tanggal'', t.Total as ''Jumlah'',t.notes as ''Keterangan'' '+
    //  ' ,case when trans_type = '+IntToStr(TRANS_TYPE_DANA_INPUT)+' then ''Setoran''  when trans_type=  '+IntToStr(TRANS_TYPE_DANA_OUTPUT)+' then ''Pengambilan'' end as Tipe '+
    'from trs_money_trans_mst t left join mst_relation r on r.relation_id = t.relation_id '+filter;
  Result:= OpenSQL(sqL);


end;

class function TTrsMoneyTransaction.LoadRekapTabungan: TMysqlResult;
var sqL,filter: string;
begin
  filter := ' and t.trans_type in ('+IntToStr(TRANS_TYPE_DANA_INPUT)+','+IntToStr(TRANS_TYPE_DANA_OUTPUT)+') ';
  if GlobalPeriode.PeriodeAwal1 <> 0 then
    filter:= filter + ' and t.trans_date '+FormatSQLOperator(GlobalPeriode.OpPeriodeAwal1)+FormatSQLDate(GlobalPeriode.PeriodeAwal1);
  if GlobalPeriode.PeriodeAkhir1 <> 0 then
    filter:= filter + ' and t.trans_date '+FormatSQLOperator(GlobalPeriode.OpPeriodeAkhir1)+FormatSQLDate(GlobalPeriode.PeriodeAkhir1);
  ...
  sqL:=
    'select distinct '+
      't.money_trans_id, t.trans_num as ''No Trx'', t.trans_date as ''Tanggal'',r.relation_name as ''Nama Nasabah'', t.Total as ''Jumlah'',t.notes as ''Keterangan'' '+
      ' ,case when trans_type = '+IntToStr(TRANS_TYPE_DANA_INPUT)+' then ''Setoran''  when trans_type=  '+IntToStr(TRANS_TYPE_DANA_OUTPUT)+' then ''Pengambilan'' end as Tipe '+
    'from trs_money_trans_mst t left join mst_relation r on r.relation_id = t.relation_id '+filter;
  Result:= OpenSQL(sqL);
...
class function TTrsMoneyTransaction.LoadRekapTabunganNasabah: TMysqlResult;
var sqL,filter: string;
begin
{  if GlobalPeriode.PeriodeAwal1 <> 0 then
    filter:= filter + ' and t.trans_date '+FormatSQLOperator(GlobalPeriode.OpPeriodeAwal1)+FormatSQLDate(GlobalPeriode.PeriodeAwal1);
...
filter := ' and t.trans_type in ('+IntToStr(TRANS_TYPE_DANA_INPUT)+','+IntToStr(TRANS_TYPE_DANA_OUTPUT)+') ';
 ...
 if GlobalFilter.RelasiID <> 0 then
  ...
  sqL:=
    'select distinct '+
      't.money_trans_id, t.trans_num as ''No Trx'', t.trans_date as ''Tanggal'',r.relation_name as ''Nama Nasabah'', '+
      'case when trans_type = '+IntToStr(TRANS_TYPE_DANA_INPUT)+' then t.total else 0 end as ''Setoran'' ,'+
      'case when trans_type = '+IntToStr(TRANS_TYPE_DANA_OUTPUT)+' then t.total else 0 end as ''Pengambilan'' ,'+
      ' 0 as ''Saldo'','  +
      't.notes as ''Keterangan'' '+
//      ' ,case when trans_type = 1 then ''Setoran'' else ''Pengambilan'' end as Tipe '+
    'from trs_money_trans_mst t left join mst_relation r on r.relation_id = t.relation_id '+filter+
...
function TTrsMoneyTransaction.UpdatePengeluaranKas: boolean;
var i: integer; sqL: string;
begin
  try
    BeginSQL;
...
function TTrsMoneyTransaction.UpdateTabungan: boolean;
var i: integer; sqL: string;
begin
  try
    BeginSQL;
