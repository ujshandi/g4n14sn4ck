...
begin
  case vTipe of
    RELASI_TYPE_CUSTOMER: prefix:= 'C';
    RELASI_TYPE_SUPPLIER: prefix:= 'S';//UpperCase(Copy(FRelationName, 1,1))+'.';
    RELASI_TYPE_MEMBER  : prefix:= 'M';
    RELASI_TYPE_NASABAH: prefix:= 'N';
  end;
 // prefix := '';
//chan  Result:= getNextIDNum('relation_code','mst_relation',' and relation_type ='+FormatSQLNumber(vTipe),prefix,'',0);
Result:= getNextIDNum2('relation_code','mst_relation',' and relation_type ='+FormatSQLNumber(vTipe),prefix);
end;
...
begin
  sql:=
    ...
    'specification, item_code, barcode, struk_name, merk, avg_price,buying_price,selling_price, '+
    ...
    'values ('+
      ...
      FormatSQLNumber(BuyingPrice)+','+
      FormatSQLNumber(SellingPrice)+','+
...
begin
  ...
  if Result then begin
    with buffer do begin
      ...
      FItemCode    := BufferToString(FieldValue(2));
...
begin
  sqL:=
  'select i.item_id, i.item_code, i.item_name, barcode, current_stock, avg_price, buying_price,kemasan_id,merk, selling_price '+
...
//  IfThen(FVendorID <> 0, ' and vendor_id = '+FormatSQLNumber(FVendorID));
  ...
  if Result then begin
    with buffer do begin
      ...
      FSellingPrice := BufferToFloat(FieldValue(9));
   //   FItemConversion.Clear;
  //   FItemConversion.Add;
 //     FItemConversion[0].ItemId:= FItemId;
...
begin
  sqL:=
  'select i.item_id, i.item_code, i.item_name, barcode,buying_price,selling_price '+
  ...
  if Result then begin
    with buffer do begin
      ...
      FSellingPrice := BufferToFloat(FieldValue(5));
   //   FItemConversion.Clear;
    //  FItemConversion.Add;
...
'  ,(select code_name from mst_code where code_id = i.specification) as jenis '+
   ',buying_price,selling_price,discount,disabled_date '+
    'from mst_item i,  point_maping p '+
  IfThen(GlobalFilter.Numeric2<>0, ', mst_rak_item r, mst_rak_maping rm ')+
