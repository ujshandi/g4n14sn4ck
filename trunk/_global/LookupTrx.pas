...
begin
  ...
  cmbStatus.Visible := (aTableName = 'ORDER');
  Label6.Visible := (aTableName = 'ORDER');
...
begin
  ...
  {sqL:=
    ...
    'from trs_receipt_mst O where O.status_id = 1 and O.tipe = '+FormatSQLNumber(TRANS_TYPE_RECEIPT)+filter+
    ' order by O.trans_date, O.trans_num'
    ...
    'select distinct '+
      ...
      '(select relation_name from mst_relation r where r.relation_id= O.relation_id) as Customer, '+
...
begin
  ...
  else if ((NamaTable = 'RECEIPT') or (NamaTable = 'FAKTUR'))and (ACol > 4) then HAlign:= taRightJustify
