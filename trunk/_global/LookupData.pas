...
begin
  ...
  //    if FProdusenID <> 0 then filter:= filter + ' and i.produsen_id ='+FormatSQLNumber(FProdusenID);
      ...
      MySQLResult:= OpenSQL(
      'select distinct i.item_id, item_code as ''Kode'', item_name as ''Nama'', merk as Merk '+
      'from mst_item i, point_maping p where i.disabled_date is null and i.specification = p.code_id '+filter+
    ...
    else if NamaTable = 'NASABAH' then
      MySQLResult:= OpenSQL(
      'select relation_id, relation_code as ''Kode'', relation_name as ''Nama'',address1 as ''Alamat'' '+
      'from mst_relation where relation_type ='+FormatSQLNumber(RELASI_TYPE_NASABAH)+
      IfThen(AdvEdit1.Text <> '', ' and relation_name like '+FormatSQLString('%'+AdvEdit1.Text+'%'))+
      ' order by relation_name ')
