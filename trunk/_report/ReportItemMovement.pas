...
begin
  try
    ...
    for i:= 1 to item.RecordCount do begin
      ...
      IfThen(BufferToFloat(item.FieldValue(4)) <> 0, ' '+
      TMstItemConversion.GetCodeUnit(BufferToInteger(item.FieldValue(0))));
...
begin
  ...
  grid.Cells[colVendor, 0]:= 'Supplier';
  grid.Cells[colProd,   0]:= 'Produsen';
