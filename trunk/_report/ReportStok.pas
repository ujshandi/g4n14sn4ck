...
begin
  ...
  try
    ...
    grid.ColCount := data.FieldsCount+3;
    ...
    //  for j:= 0 to data.FieldsCount-1 do   begin
         grid.Cells[colCode,i]:= data.FieldValue(1);
          ...
          grid.Cells[colUnit+2,  i]:= FloatToStrFmtNull(BufferToFloat(data.FieldValue(5)));
          grid.Cells[colUnit+3,  i]:= FloatToStrFmtNull(BufferToFloat(data.FieldValue(4))*BufferToFloat(data.FieldValue(5)));
          data.MoveNext;
     // end;
    end;
    data.destroy;
    grid.Cells[colUnit+3,grid.RowCount-1]:= FloatToStrFmtNull(grid.ColumnSum(colUnit+3,1,grid.RowCount-2));
...
begin
  ...
  0: if ACol >= 5 then HAlign:= taRightJustify;
  ...
  //    end;
     ...
     // grid.Cells[colPakai,    i]:= FloatToStrFmtNull(BufferToFloat(item.FieldValue(8)));
      ...
      grid.Cells[colAkhir,i]:= FloatToStrFmt(
        grid.Floats[colAwal, i] + grid.Floats[colBeli, i] + grid.Floats[colReturDr, i]  - grid.Floats[colJual, i] - grid.Floats[colReturCr, i]  +
