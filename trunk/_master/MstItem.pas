...
type
  TfrmMstItem = class(TForm)
    ...
    procedure txtHBeliValueValidate(Sender: TObject; value: String;
      var IsValid: Boolean);
    procedure txtHJualValueValidate(Sender: TObject; value: String;
      var IsValid: Boolean);
  private
    MstItem: TMstItem;
    MstUnit: TMstMaster;
...
begin
  ...
  MstItem.Barcode   := txtCode.Text;
  MstItem.ItemName  := txtName.Text;
  MstItem.StrukName := txtName.Text; //txtStruk.Text;
...
//  if purpose = 1 then
    MstItem.BuyingPrice := StrFmtToFloatDef(txtHBeli.Text,0);
  //if purpose = 2 then
    MstItem.SellingPrice := StrFmtToFloatDef(txtHJual.Text,0);  
   {
...
//  if CekInput(Result, txtBarcode.Text <> '', 'Barcode belum diisi.', txtBarcode) then
  ...
  if CekInput(Result, ((not MstItem.isExistName(txtName.Text)) and (txtCode.Tag = 0)) or (txtCode.Tag <> 0), 'Nama Item Sudah ada.', txtName) then
...
begin
  ...
  txtHBeli.Text := FloatToStrFmt(MstItem.BuyingPrice);
  txtHJual.Text := FloatToStrFmt(MstItem.SellingPrice);
  setEnableDisabled;
  ArrangeColsize;
...
procedure TfrmMstItem.txtHBeliValueValidate(Sender: TObject;
  value: String; var IsValid: Boolean);
begin
  AdvEdtFloatFmt(Sender, value);
  if IsValid then
...
procedure TfrmMstItem.txtHJualValueValidate(Sender: TObject; value: String;
  var IsValid: Boolean);
begin
      AdvEdtFloatFmt(Sender, value);
        if IsValid then
