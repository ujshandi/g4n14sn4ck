...
begin
  ...
  grid.Cells[colDisable,0] := 'Status';
  ArrangeColWIdth;
...
begin
  try
    ...
    for i:= 1 to item.RecordCount do begin
      ...
      //if (BufferToDateTime(item.FieldValue(12))>0)
...
//  ', 4merk,5struk_name,6round(i.current_stock), '+
  ///7sat,
      ...
      grid.Cells[colJenis,   i]:= BufferToString(item.FieldValue(8));
       ...
       grid.Cells[colDisable, i] := IfThen(BufferToDateTime(item.FieldValue(12))>0,'Tidak Aktif','Aktif');
...
begin
  ...
  if (ACol = colDisable) then HAlign := taCenter;
end;
