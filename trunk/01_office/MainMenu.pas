...
uses About, ChangePassword, HtmlHlp, Login,
  ...
  MstAkun, TrsItemUsetList, TrsItemUse, ReportProfit, trsSalesGania,
  trsReturSales, trsPembayaran, MoneyTransaction, ReportDebt,
  ReportPembelian, ReportGiro, trsTransaksiKasList, PengeluaranKas;
...
procedure InitializeApplication;
begin
  Application.Name     := 'Gania';
  Application.HelpFile := 'Gania.chm';
  Application.Title    := 'Gania Snack '+DATE_VERSION +IfThen(DEMO_VERSION, ' [demo version]');
end;
  ...
  if (SystemMenu.FormType in [ftMaster,ftTransaction, ftReport,ftSetting])
    ...
    if not (Can_Access(Form, AMenuID)) then  Exit;
      ...
      case AMenuID of
        ...
        330: Form.Execute(TRANS_TYPE_SALES); //frmTrsItemShipmentList
        331: Form.Execute(TRANS_TYPE_SALES_RETUR); //frmTrsItemShipmentList
        ...
        400: Form.Execute(TRANS_TYPE_PEMBAYARAN); //TRANS_TYPE_PEMBAYARAN utang
        401: Form.Execute(TRANS_TYPE_PIUTANG);
        402: Form.Execute(TRANS_TYPE_DANA_INPUT);//tabungan
        403: Form.Execute(TRANS_TYPE_PENGELUARAN_KAS);//tabungan
  ...
  case tag of
   ...
   begin
    ...
    begin
     ...
     SALES_TYPE_RETUR :
    begin
      Application.CreateForm(TfrmTrsReturSales, frmTrsReturSales);
      temp:= frmTrsReturSales;
    end;
...
{    320:
    begin
      Application.CreateForm(Tfrm, frmTrsItemUseList);
   ...
   403 : begin
      Application.CreateForm(TfrmTrsTransaksiKasList, frmTrsTransaksiKasList);
      temp:=frmTrsTransaksiKasList;
   end;
    TRANS_TYPE_PENGELUARAN_KAS: begin
      Application.CreateForm(TfrmPengeluaranKas, frmPengeluaranKas);
      temp:=frmPengeluaranKas;
   end;
   TRANS_TYPE_PEMBAYARAN,TRANS_TYPE_PIUTANG : begin
      Application.CreateForm(TfrmTrsPembayaran, frmTrsPembayaran);
      temp:=frmTrsPembayaran;
   end;
   
  { TRANS_TYPE_DANA_INPUT,TRANS_TYPE_DANA_OUTPUT : begin
      Application.CreateForm(TfrmMoneyTransaction, frmMoneyTransaction);
      temp:=frmMoneyTransaction;
   end;}
    501:
    begin
    ...
    504:
    begin
      Application.CreateForm(TfrmReportPembelian, frmReportPembelian);
      temp:=frmReportPembelian;
    end;
    ...
    525:
    begin
      Application.CreateForm(TfrmReportGiro, frmReportGiro);
      temp:= frmReportGiro;
    end;
