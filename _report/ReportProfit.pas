...
begin
  try
    ...
    if (chkKredit.Checked) then begin
     ...
     // hpp:= tmp;
      ...
      row:= InsertReportLabel('','Pembayaran Kredit');
      ProsesProgress(70,100);
      tmp:= TReport.getPembayaran(TRANS_TYPE_PEMBAYARAN);
      // karena yg lain ditutup maka
      hpp := tmp;
      //hpp:= hpp - tmp;
      grid.Floats[colValue1, row]:= tmp;
   end;

     row:= InsertReportLabel('','Pembelian (lgs)');
    ProsesProgress(50,100);
    tmp:= TReport.getPembelian(TRANS_TYPE_BELI_LANGSUNG);
    hpp:= hpp+tmp;
    grid.Floats[colValue2, row]:= tmp;

    row:= InsertReportLabel('','Potongan Pembelian (lgs)');
    ProsesProgress(70,100);
    tmp:= TReport.getPembelianDisc(TRANS_TYPE_BELI_LANGSUNG);
    hpp:= hpp - tmp;
    grid.Floats[colValue1, row]:= tmp;
    ...
    grid.RowHeights[row]:= 10;
    biaya:= 0;
    row:= InsertReportLabel('Pengeluaran Kas');
...
//    for i:= 1 to akun_biaya.RecordCount do begin
  //    row:= InsertReportLabel('', BufferToString(akun_biaya.FieldValue(1)));
      tmp:= TReport.getBiaya(5);//static ke row -lainlain  BufferToInteger(akun_biaya.FieldValue(0)));
      biaya:= biaya + tmp;
      //grid.Floats[colValue1, row]:= tmp;
     // akun_biaya.MoveNext;
   // end;
