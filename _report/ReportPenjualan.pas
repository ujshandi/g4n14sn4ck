...
begin
  try
    ...
    for i:= 1 to tgl.RecordCount do begin
      ...
      grid.Cells[colJualBarang,   i+1]:= FloatToStrFmtNull(BufferToFloat(item.FieldValue(4)));
      grid.Cells[colJualBarangPokok,   i+1]:= FloatToStrFmtNull(BufferToFloat(item.FieldValue(7)));
      grid.Cells[colJualBarangProfit,   i+1]:= FloatToStrFmtNull(BufferToFloat(item.FieldValue(4))-BufferToFloat(item.FieldValue(7)));
...
//      grid.Cells[colNet,    i+1]:= FloatToStrFmtNull(BufferToFloat(item.FieldValue(0)) + BufferToFloat(item.FieldValue(1)));
      grid.Cells[colNet,    i+1]:= FloatToStrFmtNull(BufferToFloat(item.FieldValue(4)) + BufferToFloat(item.FieldValue(5))+ BufferToFloat(item.FieldValue(6))
                -BufferToFloat(item.FieldValue(1)));
      grid.Cells[colProfit, i+1]:= FloatToStrFmtNull(BufferToFloat(item.FieldValue(2)));
      grid.Cells[colCust,   i+1]:= FloatToStrFmtNull(BufferToFloat(item.FieldValue(3)));
...
begin
  ...
  grid.Cells[colJualBarangProfit,  1]:= 'Profit';
...
begin
    ...
    { // for j:= 1 to colSubtotal do begin
     ...
     // end;     }
      ...
      //  grid.Cells[colStuk, grid.RowCount-1]:= item.FieldValue(17);
        ...
        grandTotalPokok := grandTotalPokok + IfThen(BufferToInteger(item.FieldValue(18))=SALES_TYPE_RETUR,-1,1)*TotalPokok;
        TotalPokok := 0;
  ...
  {  grid.Cells[colDiscRp,grid.RowCount-1]:= 'TUNAI :';
    grid.Cells[colSubtotal,grid.RowCount-1]:= FloatToStrFmtNull(grandTunai);
     ...
     grid.AddRow;}
    grid.Cells[colDiscRp,grid.RowCount-1]:= 'Grand Tot.:';
    grid.Cells[colSubtotal,grid.RowCount-1]:= FloatToStrFmtNull(grandTotal);
     grid.AddRow;
    grid.Cells[colDiscRp,grid.RowCount-1]:= 'Grand Tot.Pokok:';
