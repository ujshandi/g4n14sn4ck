...
uses
  ...
  ExtCtrls, SUIImagePanel,UPetshop,UTransaction,frmDockForm;

type
  TfrmPengeluaranKas = class(TDockForm)
    ToolBar1: TToolBar;
    tbtNew: TToolButton;
    tbtSave: TToolButton;
...
procedure TfrmPengeluaranKas.ArrangeColSize;
begin
  grid.AutoSizeColumns(True,2);
  grid.ColWidths[colDesc] :=300;
  grid.ColWidths[colAkun] :=0;
...
procedure TfrmPengeluaranKas.Calculate;
var
  total: double;
  i: integer;
begin
...
procedure TfrmPengeluaranKas.InitForm;
begin
...
procedure TfrmPengeluaranKas.InitGrid;
begin
   ResetGrid(grid,2,colID+2,1,1,-1);
   ArrangeColSize;
end;

function TfrmPengeluaranKas.isSaved: boolean;
var i:integer;
begin
   Trx.Reset;
   ...
   for i:= 1 to grid.RowCount-1 do begin
      if (not IsClear(grid, colDesc, i)) and (grid.Ints[colModID, i] <> 0) and (grid.Floats[colAmount, i] <> 0) then begin
        Trx.trsDetail.Add(0,lblTransaksi.Tag,MONEY_TYPE_KAS,grid.Cells[colDesc,i],grid.Floats[colAmount,i],5);//lain"
      end;
   end;
   if lblTransaksi.Tag=0 then
     Result := Trx.InsertPengeluaranKas
   else
     Result := Trx.UpdatePengeluaranKas;
end;

function TfrmPengeluaranKas.isValid: boolean;
...
procedure TfrmPengeluaranKas.SetDisableEnable;
begin

end;

procedure TfrmPengeluaranKas.tbtNewClick(Sender: TObject);
var tag : integer;
begin
  tag := 401;
...
procedure TfrmPengeluaranKas.rgTipeClick(Sender: TObject);
begin
  //purpose:=rgTipe.ItemIndex+1;
//  TMstAkun.getList(ls,purpose);
end;

procedure TfrmPengeluaranKas.gridCanEditCell(Sender: TObject; ARow,
  ACol: Integer; var CanEdit: Boolean);
begin                 //colAKun
  CanEdit := ACol in [colDesc,colAmount];
end;

procedure TfrmPengeluaranKas.gridCellValidate(Sender: TObject; Col,
...
procedure TfrmPengeluaranKas.gridGetAlignment(Sender: TObject; ARow,
...
procedure TfrmPengeluaranKas.gridGetEditorType(Sender: TObject; ACol,
...
procedure TfrmPengeluaranKas.gridKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
...
procedure TfrmPengeluaranKas.gridKeyPress(Sender: TObject;
  var Key: Char);
begin
  case grid.Col of
...
procedure TfrmPengeluaranKas.tbtSaveClick(Sender: TObject);
begin
  if isValid then
...
procedure TfrmPengeluaranKas.FormClose(Sender: TObject;
...
procedure TfrmPengeluaranKas.LoadData;
...
procedure TfrmPengeluaranKas.gridComboChange(Sender: TObject; ACol, ARow,
  AItemIndex: Integer; ASelection: String);
begin
...
procedure TfrmPengeluaranKas.Execute(vPurpose, ID: integer);
begin
  inherited;
 purpose:= vPurpose;
