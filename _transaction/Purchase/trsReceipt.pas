...
begin
  ...
  Self.Caption := 'Pembelian Barang';
  lblTransaksi.Tag:= 0;
  txtNotes.Tag:= 0;
  txtSpec.Tag:= 0;
  txtOrder.Tag:= 0;
  chkBeliLangsung.Enabled := true;
  chkBeliLangsung.Checked := True;
...
begin
  ...
  grid.ColWidths[colBarcode     ]:= 0;//65;
...
begin
  ...
  Trx.Tipe       := IfThen(chkBeliLangsung.Checked,TRANS_TYPE_BELI_LANGSUNG,TRANS_TYPE_RECEIPT);
...
begin
  ...
  Trx.ReceiptId:= ID;
  Trx.SelectInDB;
  chkBeliLangsung.Checked := Trx.Tipe=TRANS_TYPE_BELI_LANGSUNG;
  chkBeliLangsungClick(nil);
...
begin
  if TTrsReceipt.PostTrans(lblTransaksi.Tag,purpose) then
    txtNotes.Tag:= 1;
  SetDisableEnable;
end;
...
begin
  ...
  if beli then begin
    ...
    purpose := TRANS_TYPE_BELI_LANGSUNG;
  end
  else begin
    txtOrder.Clear;
    txtOrder.Tag:=0;
    btnOrder.Enabled:= True;
    purpose := TRANS_TYPE_RECEIPT;
  end;
end;
